...
class DocflowApp:
    ...
    def _update_markdown_docflow_last_read_for_library_path(self, abs_path: Path, payload: dict[str, object]) -> None:
        ...
        md_path, html_path = self._resolve_markdown_docflow_last_read_target(abs_path)
        if md_path is None:
            return

        original_stat = md_path.stat()
        changed = update_markdown_docflow_last_read(md_path, updated_at, html_path=html_path)
        if changed:
            os.utime(md_path, (original_stat.st_atime, original_stat.st_mtime))

    def _resolve_markdown_docflow_last_read_target(self, abs_path: Path) -> tuple[Path | None, Path | None]:
        suffix = abs_path.suffix.lower()
        if suffix == ".md":
...
def _pdf_media_filter_lua(*, keep_svg: bool, allow_remote_images: bool = True) -> str:
    ...
    return (
        ...
        "  return out\n"
        "end\n"
        "local function is_known_extensionless_remote_image_src(path)\n"
        "  if path:match('^https?://res%.cloudinary%.com/.+/image/upload/') then\n"
        "    return true\n"
        "  end\n"
        "  if path:match('^https?://res%.cloudinary%.com/.+/image/fetch/') then\n"
        "    return true\n"
        "  end\n"
        "  return false\n"
        "end\n"
        "local function is_known_proxy_image_src(path, ext)\n"
        "  if is_known_extensionless_remote_image_src(path) then\n"
        "    return true\n"
        "  end\n"
        "  if ext == 'webp' and path:match('^https?://substackcdn%.com/image/fetch/') then\n"
        "    return true\n"
        "  end\n"
        "  return false\n"
        "end\n"
        ...
        "      return KEEP_SVG\n"
        "    end\n"
        "    if is_known_proxy_image_src(path, ext) then\n"
        "      return true\n"
        "    end\n"
