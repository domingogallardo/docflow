"""Build intranet Reading index pages under BASE_DIR/_site/reading."""
...
# Support direct execution: `python utils/build_reading_index.py ...`
...
READING_BASE_STYLE = (
    ...
    "ul{margin-top:0}"
    ".dg-reading-list{list-style:none;padding-left:0}"
    ".dg-reading-list li{padding:2px 6px;border-radius:6px;margin:2px 0;display:flex;justify-content:space-between;gap:10px;align-items:center}"
...
class SiteReadingItem(NamedTuple):
    rel_path: str
    name: str
    mtime: float
    sort_mtime: float
...
def collect_site_reading_items(base_dir: Path) -> list[SiteReadingItem]:
    ...
    reading_items = state_items if isinstance(state_items, dict) else {}

    items: list[SiteReadingItem] = []
    for rel in sorted(reading_items):
        try:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build intranet Reading index pages.")
    parser.add_argument("--base-dir", help="BASE_DIR with Incoming/Posts/Tweets/... and _site/")
    parser.add_argument("--output-dir", help="Output dir (default BASE_DIR/_site/reading)")
...
def main(argv: list[str]) -> int:
    ...
    out_path = write_site_reading_index(base_dir, out_dir)
