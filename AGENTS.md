...
- Utilities: `utils/` for helper scripts (for example `docflow_server.py`, `build_browse_index.py`, `build_reading_index.py`, `build_done_index.py`).
...
- `BASE_DIR` must come from `DOCFLOW_BASE_DIR` in `~/.docflow_env`; do not hardcode it in `config.py`.
