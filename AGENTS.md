...
This repository automates collecting and organizing personal documents (Instapaper posts, Snipd podcasts, Markdown notes, PDFs, tweets vía pipeline dedicado). Keep changes small, tested, and aligned with the current modular design.
...
- Tweets queue: `python process_documents.py tweets` abre tus likes de X con Playwright (`TWEET_LIKES_STATE`, `TWEET_LIKES_URL`) hasta encontrar el último tweet registrado en `Incoming/tweets_processed.txt`, evita duplicados y completa el pipeline Markdown/HTML
