...
> - The bundled `web/nginx.conf` includes FancyIndex for `/docs/` and `/posts/`, and uses `server_name localhost` inside the container.
...
- **[APP] Nginx in Docker container**  
  ...
  - Exposes **/docs** (and optionally **/posts**) with **FancyIndex** ordered by **mtime desc** (ideal for “bumps”).  
  - Provides **/data** for simple edits via **HTTP PUT** (no delete) protected by **BasicAuth** using a host‑mounted `.htpasswd`.

**Why mtime/fancyindex?**  
Setting a file’s modification time (mtime) lets you “bump” items so they float to the top in the directory listing. FancyIndex can sort by date descending, which pairs nicely with this workflow.
...
└─ public/                # your static site (index.html, /docs, /posts, etc.)
```

### 5.2 Dockerfile (Alpine + FancyIndex + TZ)
...
#
# Nota: FancyIndex muestra listados por fecha de modificación (ideal para “bumps”).
...
```nginx
# Load dynamic modules installed via apk (FancyIndex)
...
http {
    ...
    server {
        ...
        # Bump-friendly FancyIndex (sorted by mtime desc)
        ...
        # Optional: mirror same behavior for /posts
        # location = /posts { return 301 /posts/; }
        # location /posts/ {
