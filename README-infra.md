...
> - The bundled `web/nginx.conf` serves `/docs/` and `/posts/` via static `index.html` files generated at deploy time, and uses `server_name localhost` inside the container.
...
- **[APP] Nginx in Docker container**  
  ...
  - Exposes **/docs** and **/posts** with static indexes ordered by **mtime desc** (ideal for “bumps”), generated by the deploy script.
...
http {
    ...
    server {
        ...
        # Docs: static index (mtime desc, generated at deploy)
        ...
        # Posts: static index (mtime desc, generated at deploy)
