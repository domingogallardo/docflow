...
- On `POST /api/to-done`, `working_at` and bump `updated_at` (when present) are copied into `done.json` as `working_started_at` and `bumped_started_at` so lead time can be computed after completion.
