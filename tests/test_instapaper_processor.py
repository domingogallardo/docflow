...
def test_instapaper_processor_download_problematic_images(tmp_path):
    """Descarga solo imágenes remotas que devuelven 403."""
    ...
    html_file = incoming / "medium.html"
    html_file.write_text("<html><body><img src=\"http://example.com/protected.jpg\"></body></html>")
    ...
    png_bytes = (
        ...
        b"\x18\xdd\x8e\x7f\x00\x00\x00\x00IEND\xaeB`\x82"
    )

    def fake_get(url, headers=None, **kwargs):
        if headers and headers.get('Referer'):
            return Mock(status_code=200, content=png_bytes, headers={'content-type': 'image/png'})
        return Mock(status_code=403, headers={'content-type': 'text/html'})

    with patch('instapaper_processor.requests.get', side_effect=fake_get):
        processor._download_problematic_images()

    updated_content = html_file.read_text()
    assert 'protected.jpg' not in updated_content
    local_images = list(incoming.glob('medium_img*.png'))
    assert len(local_images) == 1
    assert local_images[0].name in updated_content


def test_downloaded_images_moved_with_html(tmp_path):
    """Las imágenes descargadas se mueven junto al HTML manteniendo la referencia."""
    ...
    posts = processor._list_processed_files()
    processor._move_files_to_destination(posts)

    moved_html = destination / "medium.html"
    moved_img = destination / "medium_img0.png"
    assert moved_html.exists()
    assert moved_img.exists()
    assert moved_img.name in moved_html.read_text()
